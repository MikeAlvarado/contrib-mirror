...
Contribution Mirror is a composite GitHub Action that mirrors the daily activity of another GitHub account, typically a work account, into the repository where it runs. Every active day on the source account becomes one commit dated to that day, so the activity shows on the graph of the account that owns the repository. Only dates are stored: no repository names, no commit messages, no counts, nothing else from the source account.
...
| `log-file` | no | `log.md` | File that records mirrored dates, one per line. |
| `timezone` | no | `UTC` | IANA timezone name, used only for the commit timestamp. |

| Output | Description |
| --- | --- |
| `mirrored-days` | Number of days mirrored in this run. |

## Backfill

Open the Actions tab, pick the mirror workflow, choose "Run workflow", and set `lookback-days` to `365`. Days already present in the log file are skipped, so running a backfill more than once is safe.

## Privacy and policy

What is stored in the destination repository:

- The log file, containing one `YYYY-MM-DD` line per mirrored day.
- One commit per mirrored day, whose message is that date and whose timestamp is noon of that day in the configured timezone.

What is never stored: repository names, commit messages, contribution counts, activity levels, or any other detail from the source account.
...
- If GitHub changes the contribution graph markup, the run fails instead of silently mirroring nothing. Zero parsed cells is treated as an error.
